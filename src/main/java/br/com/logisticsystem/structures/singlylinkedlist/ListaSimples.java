...
public class ListaSimples<T> {
    ...
    private void validarPosicao(int posicao) {
        if (posicao < 0 || posicao >= this.tamanho) {
            throw new PosicaoInvalidaException("Posição inválida: " + posicao + ". Tamanho atual: " + this.tamanho);
        }
    }

    private void validarPosicaoInsercao(int posicao) {
        if (posicao < 0 || posicao > this.tamanho) {
            throw new PosicaoInvalidaException("Posição de inserção inválida: " + posicao + ". Tamanho atual: " + this.tamanho);
        }
    }
    ...
    public Object[] listar() {
        ...
        while (atual != null) {
            elementos[indice++] = atual.getElemento();
            atual = atual.getProximo();
        }
